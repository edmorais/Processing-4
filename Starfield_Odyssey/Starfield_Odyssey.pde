 ...
 * STARFIELD ODYSSEY (from Genuary 2023! 29)
 * by Eduardo Morais 2023 - www.eduardomorais.pt
 */

// config
int   $appWinWidth   = 1080;
...
float MAXSPD = 4, MAXSCALE = 5, MOUSEF = 8, JITTER = 0.25;
int FLOWER_NUM = 64, POLY_NUM = 64, STAR_NUM = 16, POINT_NUM = 600;
// internal
int SHAPE_POLYGON = 0;
int SHAPE_FLOWER = 1;
int SHAPE_STAR = 2;
...
void setup() {
  ...
  $fr = createGraphics(width,height/2);
  background($appWinBG);
...
void draw() {
  ...
  for (int i=0; i<points.length; i++) {
    points[i].draw($fr, mx,my,128);
  } 
  
  $fr.blendMode(SCREEN);
  for (int i=0; i<polys.length; i++) {
    polys[i].draw($fr, mx,my,16);
  } 
